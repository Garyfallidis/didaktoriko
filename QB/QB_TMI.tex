...
\begin{abstract}
  ...
  collectively these representative tracks can be taken as a complete
  distillation of the tractography. Representative tracks can then be
  used as input to clustering algorithms of higher order complexity
  which currently cannot be implemented on the full data set. We also
  show the further potential of QB to find landmarks, create atlases,
  and compare and register tractographies.
\end{abstract}
...
all pairwise distances unlike most of the other existing
...
\begin{algorithmic}
\REQUIRE $T=\{\mathbf{s}_{1},...,\mathbf{s}_{i},...,\mathbf{s}_{|T|}\}$, $\theta$
\ENSURE $C=\{c_{1},...,c_{k},...,c_{|C|}\}$ 
\STATE \# create first cluster
\STATE $c_{1} \leftarrow \left\{[1],\mathbf{s}_{1},1\right\}$
\STATE $C\leftarrow\left\{c_{1}\right\}$
\STATE $|C|\leftarrow1$ 
\FOR{$i=2$ to $|T|$}
...
\STATE \# create new cluster
	...
	\STATE $C_{|C|}.\mathbf{h}\leftarrow \mathbf{t}$
	\STATE $C_{|C|}.N\leftarrow 1$
	\STATE \textbf{append}($C_{|C|}.I$,$i$)
\ENDIF
\ENDFOR 
\end{algorithmic}
...
have $|C|$ clusters. Select the first track $\mathbf{s}_{1}$ and place it in
the first cluster $c_{1}\leftarrow(\{1\},\mathbf{s}_{1},1)$.  Obviously $|C|
...
tracks $\mathbf{v}_{e}$ of all the current clusters $c_{e}$, $e = 1 \dots |C|$,
...
$c_{e}\leftarrow(I\cup\{i\},\mathbf{h}+\mathbf{s},n+1)$; otherwise create a new
cluster $c_{|C|+1}\leftarrow(\{i\},\mathbf{s}_{i},1)$, $|C|\leftarrow|C|+1$.
...
track $s\mathbf{s}$ and track $\mathbf{s'}$, see Eq.~(\ref{eq:direct_flip_distance}). This
...
d_{\textrm{flipped}}(s,s') & = & \frac{1}{K}\sum_{i=1}^{K}|\mathbf{x}_{i}-\mathbf{x}_{K-i}'|.\nonumber
\end{eqnarray}
\noindent
...
distance) to the virtual track. We will call this exemplar track $\mathbf{e}_{1}$,
i.e.~$\mathbf{e}_{1}={\displaystyle \argmin_{\mathbf{x} \in C_i}}\textrm{\,\ MDF}(\mathbf{v},\mathbf{x})$.
...
tracks in the bundle, which we denote by $\mathbf{e}_{2}={\displaystyle
  \argmin_{\mathbf{x} \in C_i}}\,{\displaystyle \sum_{y\in C_i}}\mathrm{MDF}(\mathbf{y},\mathbf{x})$, or
if we want to work with tracks with possibly different numbers of points
we could instead use $\mathbf{e}_{3}={\displaystyle \argmin_{\mathbf{x} \in
    C_i}}\,{\displaystyle \sum_{\mathbf{y}\in C_i}}\mathrm{MAM}(\mathbf{y},\mathbf{x})$.
...
approach with a novel metric which we call Bundle Adjacency (BA). This metric works as follows. Let us assume that we have gathered the
exemplar tracks from clustering A in $E_{A}=\{\mathbf{e}_{1},...,\mathbf{e}_{|E_{A}|}\}$
and from clustering B in $E_{B}=\{\mathbf{e}_{1}^{'},...,\mathbf{e}_{|E_{B}|}^{'}\}$ where
...
keep those distances that are smaller than a tight threshold
$\theta$. Then we define BA to be

\begin{equation}
BA=\frac{1}{2}\left(\frac{|E_{A\rightarrow B}\leq \theta |}{|E_{A}|}+\frac{|E_{B\rightarrow A}\leq \theta |}{|E_{B}|}\right)\label{eq:TC}
\end{equation}
...
$\theta$. In other words, $BA$ is mean of the fraction of row minima of
$D_{AB}$ that are less than $\theta$ and the fraction of column minima
less than $\theta$.  When $BA=0$ that means that every exemplar from the
one set was further than $\theta$ to all exemplars in the other
set. When $BA=1$ then all exemplars from one set had a close neighbour
...
to $|V_{B}|$. (2) We merge those clusters from B whose virtual
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.5]{Fig_7_close_distance}
\par\end{centering}

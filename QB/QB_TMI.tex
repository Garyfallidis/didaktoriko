...
We first justify our claims about the speed and linear complexity of QB
(\ref{sub:Complexity}) and about the robustness of QB to changes in
ordering of the tractography (\ref{sub:Comparisons}). Next in section
\ref{QB_Representation} we demonstrate the high level of clarity and
simplification that the QB representation can bring to a tractography,
and how adjustment of the distance threshold $\theta$ can be use to
merge or further disaggregate clusters.

Alignments, atlases and landmarks (\ref{sub:Atlases-made-easy})

QB as input to other clustering methods (\ref{sub:QB_as_input})

Exemplars improve on use of ROIs or masks (\ref{sub:exemplars_vs_ROIs})

Direct Tractography Registration (\ref{sub:direct_registration})

Strategies with short tracks (\ref{sub:short_tracks}) 
...
\subsection{QB as input to other clustering methods\label{sub:QB_as_input}}
...
\subsection{Exemplars improve on use of ROIs or masks\label{sub:exemplars_vs_ROIs}}
...
\subsection{Strategies with short tracks\label{sub:short_tracks}}
...
\begin{figure*}
\centering\includegraphics[scale=0.95]{Fig_1_QB_algorithm}

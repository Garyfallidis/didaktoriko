...
discretization of streamlines has been implemented, where all streamlines
have the same number of points $K$, and all segments of
each streamline have equal length.
...
\caption{QB clustering of the Fornix bundle. The original Fornix
  ...
  all cases the streamlines of the Fornix were discretized to have $18$
...
streamlines in the tractography are permuted. However this statistic has
...
points of the full dataset should be near to the reduced set (coverage),
but not too many (overlap).
...
$t \in T$ with $\mathrm{MDF}(s,t)\le \theta$. We define the
\emph{coverage} of $S$ by $T$ as the fraction of $S$ that is
$\theta$-approximated by $T$. Coverage ranges between 0 (worst case,
when all streamlines in $S$ are too far from $T$) and 1 (best case, when
every streamline in $S$ is $\theta$-approximated by $T$).

However we recognise that while $T$ may have good coverage of $S$ there
may be some redundanc, so we define the \emph{overlap} of $T$ in $S$ as
the average number of approximating streamlines in $T$ for
$\theta$-approximated streamlines in $S$.  Overlap is undefined if no
streamlines in $S$ are approximated by $T$, otherwise it can take any
value greater then or equal to 1, with higher values indicating greater
degrees of redundancy of $T$ in $S$; if $T$ has several similar
streamlines then this will tend to boost overlap.

Coverage and overlap will typically measure how well a smaller set of
streamlines acts as a reduction of a larger set. In order to compare two
reductions of possibly different datasets we define the symmetric
measure \emph{bundle adjacency} (BA).  BA is the average of the coverage
of $T$ by $S$ and the coverage of $S$ by $T$: $$\mathrm{BA}(S,T) =
...
will examine the stability of QB in this respect.

As a first step we recorded the numbers of QB clusters in $20$ different
random permutations of the tractographies of $10$ human subjects as
described in section \ref{sub:QB-Data-sets}. We first removed short
streamlines shorter than $40$~mm and down-sampled them to $12$
points. Then we applied QB with threshold at $10$~mm to the permuted
data sets. The average mean number of clusters was $2645.9$ (min
$1937.6$; max $3857.8$; s.d.~$653.8$; $N=12$). There is therefore a
considerable between-subject variation in the number of clusters. By
contrast the within-subject variability of the number of clusters across
random permutations is rather small, with average standard deviation
$12.7$ (min $7.3$; max $17.4$). This suggests that QB may be achieve
some degree of consistency in terms of the numbers of derived clusters,
and in a manner that may reflect between subject differences. However
this does not say anything about the similarity of these clusterings.

So next we investigated how consistent the QB clusterings are when
datasets are permuted. Twelve different random permutations were
generated for each of $10$ tractographies and the corresponding QB
clusterings were computed with MDF threshold $10$~mm. For each subject
the $66$ pairings of QB clusterings were compared using the optimised
matched agreements (OMA) index and then averaged. Across subjects the
mean OMA (\ref{sub:Tightness-comparisons-1}) was 74.1\% ($\pm 0.39$\%)
which can be interpreted as a good level of
...
$10$~mm and discretizing of $18$ points. For every subject we

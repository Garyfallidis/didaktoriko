...
It is called the \emph{optimised matched agreement} (OMA). As with the \emph{adjusted
Rand index}~\citep{moberts2005evaluation}, OMA requires the calculation
of the $M \times N$ streamline cross-classification matrix $X=(x_{ij})$. This
counts the number of streamlines in all pairwise intersections of
...
need to find one empirically. If $j=\pi(i)$ is a possible matching then the corresponding
\emph{matched agreement} is $\mathrm{MA}(\pi) = \sum_{i=1}^M x_{i,\pi(i)}$. A
...
Besides the considerable computational overheads in calculating the
cross-classification matrix, there is a further fundamental disadvantage of these
...
\emph{coverage}, \emph{overlap} and \emph{bundle adjacency} (BA).
...
$\theta$-neighbour in $T$: $$\mathrm{coverage}(S,T) = |\{s \in S
\,\mathrm{has~a}~\theta\mathrm{-neighbour~in}~T\}|/|S|.\nonumber$$
Coverage ranges between 0 (when no streamline in S has a close enough
neighbour in T) and 1 (when every streamline in S has a neighbour in T).

We define the overlap of $T$ in $S$ as the average number of
$\theta$-neighbours in $T$ for streamlines in
$S$: $$\mathrm{overlap}(S,T)=\sum_{s \in S}|\{t \in
T:t\,\mathrm{is~a}~\theta\mathrm{-neighbour~of}~s\}|/|S|.\nonumber$$
Overlap can take any non-negative value, with higher values indicating
possible redundancy of $T$ in $S$; if $T$ has several similar
...
overlap of $S$ in $T$ will be low; and if $S$ and $T$ are globally

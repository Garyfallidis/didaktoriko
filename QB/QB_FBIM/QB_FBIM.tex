...
terminology ...”  We can prove that MDF is a metric (not just a
semi-metric): A metric space is an ordered pair  where  is a set and  is
a metric on , i.e., a function such that for any , the following holds:
(non-negative), iff   (identity of indiscernibles), (symmetry) and
(triangle inequality) .  (I started trying to rewrite this in a more
formal way but am not entirely happy with where it is going!)  A
streamline s+ is an oriented polyline (s1, s2, … , sp) in R3 and s- ,
the flip of s+, is the reverse streamline (sp, sp-1, … , s1). A track x
is an unoriented polyline [x1, x2, … , xp]. We identify a track x with
the pair of streamlines {x+, x-}. Henceforth we assume that streamlines
and tracks have the same number p of points. The (direct) distance
d(s1,s2) between two streamlines s1, and s2 is the sum of the euclidean
distances between corresponding points. The minimum distance flipped
mdf(x1, x2) between two tracks x1 and x2 is min(d(x1+, x2+), d(x1+,
x2-)). Note that d(x1+, x2+) = d(x1-, x2-) and d(x1+, x2-) =  d(x1-,
x2+).  We claim that the space of p-tracks is a metric space when
equipped with mdf. The first three axioms are clearly satisfied. To
prove the triangle inequality let us assume that x, y and z are tracks
and that, without loss of generality, d(x+, y+) and d(y+, z+) actually
minimise the corresponding mdf distances. Then mdf(x, y) + mdf(y, z) =
d(x+, y+) + d(y+, z+)  d(x+, z+) (by the triangle inequality for the
euclidean distance)  min(d(x+, z+), d(x+, z-)) = mdf(x, z). QED.

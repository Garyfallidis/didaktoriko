...
Nonetheless, Moberts et al.~\citeyear{moberts2005evaluation} are an
...
which performed best. Wang et al.~\citeyear{wang2010tractography} proposed
...
Visser et al.~\citeyear{Visser2010} used hierarchical clustering and
...
in \citet{catani2005perisylvian}. The algorithm that we present in
this chapter also uses the minimum average flip (MDF) metric as a
measure of distance between tracks. Gerig et al.~\citeyear{gerig2004analysis}
...
Guevara et al.~\citeyear{Guevara2010} combined a great number of
different algorithms from hierarchical clustering to 3D watershed on
track extremities.  They first divided the tractography into left-right
hemisphere, inter-hemispheric and cerebellum subsets. They then create
further subsets of different track length, use hierarchical clustering
...
data set between shorter and longer tracks.  Tsai et
al.~\citeyear{Tsai2007} used a combination of cluster methods based on
...
to merge a chain of neighbouring structures into one cluster. Zhang and
Laidlaw \citeyear{zhang2005dti} used an agglomerative hierarchical
...
bundles. Zvitia et al.~\citeyear{zvitia2008adaptive, Zvitia2010} used
adaptive mean shift. This is a clustering algorithm which finds
automatically the number of clusters for example in contrast with
k-means that the user needs to prespecify the number of clusters.  They
also used this approach for direct registration of tractographies but
only with tractographies from the same subject. El Kouby et
al.~\citeyear{ElKouby2005} created a ROI-based connectivity matrix where
the $i,j$th entry of the matrix holds the number of tracks which connect
$ROI_{i}$ to $ROI_{j}$. K-means was used afterwards on the rows of the
matrix to cluster the tracks. This technique can be used for clustering
bundles across subjects.

Brun et al.~\citeyear{brun2004clustering} used the mean and covariance
of the track as the feature space and normalized cuts based on a graph
theoretic approach for the segmentation. Ding et al.~\citeyear{Ding2003a}
used k-nearest neighbours, another agglomerative approach, applied
to corresponding track segments. Corouge et al.~\citeyear{corouge2004towards}
...
O'Donnell et al.~\citeyear{ODonnell_IEEETMI07} created a tractographic
...
Maddah et al.~\citeyear{Maddah_MICCA2005} used B-spline representations
of tracks referenced to an atlas, and then the tracks were clustered
based on the labeled atlas . Later Maddah et al.~\citeyear{maddah2006statistical}
...
the Corpus Callosum tracks into Witelson subdivisions \citep{witelson1989hand}
using population averages. Later in \citet{Maddah_IEEEBI2008} showed that it is possible to combine spatial priors
with metrics for the shape of the tracks in order to guide the clustering
process.

Jonasson et al.~\citeyear{jonasson2005fiber} created a large $N\times N$
...
Jianu et al.~\citeyear{jianu2009exploring} presented a new method for
...
the approximation that Chalmers~\citeyear{chalmers1996linear} introduced
for the technique of Eades~\citeyear{eades1984heuristic}.

Durrleman et al.~\citeyear{Durrleman2009} introduced electrical current
...
as shown in \citet{Durrleman2009} and later in \citet{durrleman2010registration}.
...
Leemans and Jones \citeyear{leemans17new} used affinity propagation [FORWARD REFERENCE NEEDED?] to cluster the fronto-occipital fibres,
...
Later Malcolm et al.~\citeyear{malcolm2009filtered} used affinity propagation
...
Ziyan et al.~\citeyear{ziyan2009consistency} introduced a probabilistic
...
initial spectral clustering \citep{ODonnell_IEEETMI07} to label the
...
major white matter tracts were described in Wakana et al.~\citeyear{Wakana2007NeuroImage}
using ROIs to include or exclude tracks generated by deterministic
tractography. Hua et al.~\citeyear{Hua2008NeuroImage} used regions of
...
\citet{Visser2010, gerig2004analysis, Guevara2010, zhang2005dti,
  jianu2009exploring}; \textit{$k$-means} \citet{ElKouby2005, Tsai2007};
\textit{Adaptive mean shift} \citet{zvitia2008adaptive, Zvitia2010};
\textit{Graph theoretic segmentation} \citet{brun2004clustering};
\textit{$k$-nearest neighbours} \citet{Ding2003a}; \textit{Generalized
  Procrustes Analysis and Principal Components Analysis (PCA)}
\citet{Corouge2004, corouge2004towards, Corouge2006};
% \textit{Spectral embedding} \cite{ODonnell_IEEETMI07};
\textit{EM clustering} \citet{Maddah_MICCA2005, maddah2006statistical,
  Maddah_IEEEBI2008, ziyan2009consistency}; \textit{Spectral clustering}
\cite{jonasson2005fiber, ODonnell_IEEETMI07}; \textit{Affinity
  propagation} \citet{leemans17new, malcolm2009filtered};
...
For recent overviews see \citet{ODonnell_IEEETMI07} and \citet{wang2010tractography}.
...
$k$-means~\citet{steinhaus1956division, macqueen1967some} require
iterative reassignment of items to different clusters and hence do not
run in linear time. Others such as BIRCH~\citet{zhang1997birch} have
...
provide the distance to the nearest track in $B$ of each track in $A$
($m_{A\rightarrow B}$) and similarly the minima of the columns of
$D_{AB}$ the distance to the nearest track in $A$ of each track in $B$
($m_{B\rightarrow A}$). From these correspondences we only keep those
distances that are smaller than a tight threshold $\theta$. Then we
define BA to be
...
and similarly for $|m_{B\rightarrow A}\leq \theta |$. 
In other words, $BA$ is mean of the fraction of row minima of
$D_{AB}$ that are less than $\theta$ and the fraction of column minima
less than $\theta$.  $BA=0$ when every exemplar from the
one set is further than $\theta$ to all exemplars in the other
set. $BA=1$ when all exemplars from one set have a close neighbour
in the other set. This metric is very useful especially when
...
% -withaff}$ which gave us the correct displacements. As this is rather technical
...
problem however we can use the bundle adjacency (BA) metric
explained in section \ref{sub:Tightness-comparisons-1}.  We can report
that the mean initial BA was $34.8\%\pm8.0\%$ and the mean final BA
...
% \textit{
% Here we have described a clustering technique called QuickBundles (QB) that
...
% \ENSURE $C=\{c_{0},...,c_{k},...,c_{|C|-1}\}$ where $c=\{I,\mathbf{h},N\}$ \\
...
% 	\IF{$\texttt{flip}_{l}$ is $1$} 
% 		\STATE $C_{l}.\mathbf{h}+\leftarrow t'$
% 	\ELSE
% 		\STATE $C_{l}.\mathbf{h}+\leftarrow t$
% 	\ENDIF
% 	\STATE $C_{l}.N+\leftarrow 1$
% 	\STATE \textbf{append}($C_{l}.I$,$i$)
% \ELSE 
% \STATE \# create new cluster
% 	\STATE $|C|+\leftarrow 1$
% 	\STATE $C_{|C|-1}.I_{0}\leftarrow [l]$

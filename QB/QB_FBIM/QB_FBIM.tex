...
We warped the tractographies of $10$ healthy subsects (see section \ref{sub:QB-Data-sets}) in MNI space and applied QuickBundles on each tractography using distance threshold $\theta=10$~mm and downsampling at $18$ points. For every subject we only considered the largest $100$ QB clusters i.e. the clusters which contained the highest number of streamlines. The purpose of this experiment was to identify a similarity measure between the streamlines of the different subjects. In Fig.~\ref{Flo:BAs} we observe both the complete tractographies and the centoid tracks which correspond to the $100$ largest clusters. Because the complete tractographies are very big containing hundreds of thousands of tracks we visualize them using high transparency so that at least some of the underlying structures are still visible. At the contrary the  

For this purpose we evaluated BA~(\ref{eq:TC}) between pairs of these $10$ warped tractographies. This generated $45$ BA values with $\theta=10$~mm. We also generated another $45$ BA values with $\theta=20$~mm.  
...
\begin{figure}
\begin{centering}
\includegraphics[scale=0.25]{Figures/Fig_13_BAs}
\par\end{centering}
\caption{Bundle Adjacencies for the 45 pairings of 10 subjects with different distance thresholds. \label{Flo:BAs_plot}}
\end{figure}

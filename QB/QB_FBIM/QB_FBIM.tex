...
clusterings were computed with clustering threshold $10$~mm. (These were
a subset of the clusterings used in [A].) For each subject the $120$
...
(\ref{sub:Tightness-comparisons-1}) was 72.0\% ($\pm 0.60$\%); the
average intra-subject stadard deviation was $\pm 0.63$\%. According to
the interpretation of \citep{altman1995} this represents a good level of
agreement consistent across tractographies. Bearing in mind that large
subsets of tractography streamlines present more as continuously varying
shapes rather than discrete clusters, we propose comparing with the
result of dividing a one-dimensional medium into slices of equal
thickness. Depending on the offset between two such systems of slices
the OMA will range between 100\% when they coincide and 50\% when they
symmetrically interleaved, with a mean OMA of 75\%.

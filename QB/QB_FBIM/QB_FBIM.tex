...
\citet{leemans17new, malcolm2009filtered}). However, they had to restrict
...
%data sets. Moreover methods described as `automatic' in reality require
...
reduce the size and dimensionality of the data sets as a precursor for
...
how similar two reduction are to each other. Ideally most points of the
...
% \textbf{THIS SECTION IS UNDER RECONSTRUCTION!}
...
$40$~mm were first removed and then the remaining streamlines were down
sampled to $12$ points. The mean number of retained streamlines was
\num{98159.5} ($\pm$ \num{14298.0}). Then we applied QB with a
threshold of $10$~mm. The mean number of clusters across subjects was
\num{4144.0} ($\pm$ \num{830.0}), representing a data compression
ratio of about 24:1. There is therefore a considerable between-subject
variation in this metric. Some of this inter-subject variability is
related to the sizes of the tractographies: the correlation between
number of QB clusters and full tractography size was calculated
($r=0.867$, $N=10$, threshold=10~mm); however the correlation with full
tractography size only marginally dipped to $r=0.786$ when the clustering was
performed on random samples of fixed size (\num{80000}).  This
suggests that the QB clustering reflects the complexity of the full
...
[B] Next we investigated how consistent QB clusterings are when data
sets are re-ordered. Sixteen different random permutations were
generated for each of $10$ tractographies and the corresponding QB
clusterings were computed with clustering threshold $10$~mm. For each
subject the $120$ pairings of QB clusterings were compared using the
optimized matched agreements index and then averaged. Across subjects
the average OMA (\ref{sub:Tightness-comparisons-1}) was 72.0\% ($\pm
0.60$\%); the average intra-subject stadard deviation was $\pm
0.63$\%. According to the interpretation of \citep{altman1995} this
represents a good level of agreement consistent across
tractographies. Bearing in mind that large subsets of tractography
streamlines present more as continuously varying shapes rather than
discrete clusters, we propose comparing with the result of dividing a
one-dimensional medium into slices of equal thickness. Depending on the
offset between two such systems of slices the OMA will range between
100\% when they coincide and 50\% when they symmetrically interleaved,
with a mean OMA of 75\%.

[C] Recognising that large tractography datasets present too large a
computational challenge, some authors (e.g.~\citet{ODonnell_IEEETMI07}
...
information about the full dataset. However they inevitably are
suboptimal reductions of the full dataset. Inevitably they overrepresent
the denser parts of the tractography space, and they underrepresent the
sparser regions of that space. By their construction the QB centroids
will be more uniformly distributed in this space and thus a better
compression of the original dataset.
 
[calculations under way now ...]


[at the moment this is C and D] As well as checking that QB creates sets
of centroids with good coverage and overlap statistics, we go on to
show that the performance of QB generalizes to sets of streamlines
...
with clustering threshold $20$mm and comparing against streamlines with
an adjacency threshold of 20mm. (Note that though we have selected the
same values here for the two thresholds they do not have to be the
same.) We found that with clustering threshold $10$mm the mean coverage
(s.d.) of $T_1$ by $C_1$ was $99.99$\% ($\pm 0.004$\%), of $T_2$ by
$C_1$ was $99.91$\% ($\pm 0.02$\%) and of $T_2$ by $R_1$ was $95.86$\%
($\pm 0.62$\%). The mean overlap (s.d.) at this threshold of $C_1$ in
$T_1$ was $3.54$ ($\pm 0.18$), of $C_1$ in $T_2$ was $3.54$ ($\pm
0.18$), and of $R_1$ in $T_2$ was $6.53$ ($\pm 0.93$).
...
distance often separates shorter from longer neighboring streamlines

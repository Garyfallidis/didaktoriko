...
clusters and full tractography size was calculated ($r=0.867$, $N=10$,
clustering threshold = 10~mm); however the correlation with full
tractography size only decreased marginally to $r=0.786$ when the
clustering was performed on subsets of fixed size (\num{80000}).
This suggests that the QB clustering reflects the complexity of the full
tractography not solely its size. CLARIFY
...
computed with clustering threshold $10$~mm. (These were a subset of the
clusterings used in [A].) For each subject the $120$ pairings of QB
clusterings were compared using the optimized matched agreements index
and then averaged. Across subjects the average OMA
(\ref{sub:Tightness-comparisons-1}) was 72.0\% ($\pm 0.60$\%); the
average intra-subject standard deviation was $\pm 0.63$\%. According to
\citet{altman1995} this represents a good level of agreement consistent
across tractographies. In order to quantify worst and best case
scenarios, we consider what happens when the dataset consists of
parallel lines of uniform spacing. The result of QB clustering is an approximate
partitioning into equally spaced pieces. Depending on the
offset between two such partitionings the OMA between them will range between 100\%
when they coincide and 50\% when they are most out of phase, with a mean
OMA of 75\% across the range of cases.
...
\num{10000}[?? Elef to check]) and applied their clustering algorithms
to those. We have shown in [A] above that large random subsets do
contain much of the information about the full dataset. However they are
...
compression of the original dataset. We quantify this using the coverage
and overlap statistics (\ref{sub:Tightness-comparisons-1}). Each of the
10 human subject tractographies was split into two halves, one of which
was set aside. QB clusterings were derived for remaining half. The
coverage and overlap statistics for the resulting QB centroids and for
an equal random set of streamlines are presented in Table xxxx. This was
done for two choices of values for the clustering threshold and
adjacency threshold: both 10mm, and then both 20mm.

\begin{table}[th]{QB centroids performance compared with random subsets}
\small
%\centering
\begin{tabular}{rrrr}
\hline
...
\begin{table}[th]{Performance of QB centroids tested on split halves of datasets}
\small
%\centering
\begin{tabular}{rrrr}
\hline

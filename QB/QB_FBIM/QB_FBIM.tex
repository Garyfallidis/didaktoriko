...
preferably from experts or atlases. Nevertheless, even when such
extensive priors are used this is still a supervised learning
problem. This is because anatomical bundles differ across very much both
in lengths and in shape \citet{schmahmann2009fiber} and there is not a
single threshold which can cluster a full dataset with anatomical
correspondence (see \citet{Guevara2010}). If one uses such extensive
priors then it will be wiser to use supervised learning
...
between streamlines in order to create a distance matrix. Most authors
based their work on hierarchical clustering for which they needed
dustance matrices of this type. \textbf{We found that most authors tried
  to create similar distance matrices using hierarchical clustering}
(see \citet{gerig2004analysis, moberts2005evaluation, zhang2005dti,
  Tsai2007, zhang2008identifying, jianu2009exploring, Visser2010,
  Guevara2010}) or spectral clustering (see \citet{ODonnell_IEEETMI07,
  o2009tract, jonasson2005fiber, ziyan2009consistency}) or affinity
propagation (see \citet{leemans17new, malcolm2009filtered}). However,
they had to restict themselves to only a subset of the complete dataset
because the calculations were heavy and the distance matrix too big for
computers with standard memory capacity. For example,
\citet{ODonnell_IEEETMI07} used only \num{10000} streamlines (~10\% of
the initial tractography) and \citet{Visser2010} used recombinations of
\num{10000} streamlines.  We give here an example of how large that
distance matrix can be if used with a full dataset. Just for
\num{100000} streamlines 38 GBytes are required and for 1 million
streamlines 3.6 TBytes of memory are required (4 bytes floating point
precision).

Other clustering methods have also been proposed that use graph
theoretic approaches (see \citet{brun2004clustering, ElKouby2005}),
...
\citet{Maddah_IEEEBI2008}). \textbf{Are we going to say anything about
  the time efficiency of these methods too?}

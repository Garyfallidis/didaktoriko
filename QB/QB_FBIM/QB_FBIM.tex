...
QuickBundles on each tractography independently using clustering threshold
...
clustering threshold for BA is twice that of the initial QB distance
threshold. 
...
for manually setting the clustering threshold) and interaction with the
...
has recently attracted much scientific attention; and it is recommended when large data sets are involved. QB can be used with all types of diffusion MRI
...
of 200:1 depending on the clustering threshold while preserving
...
Additionally, QB can be used to explore multiple tractographies and find correspondences or similarities between different tractographies. This can be facilitated by the use of Bundle Adjacency (BA) a new similarity measure introduced
in this paper.

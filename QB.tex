...
Hierarchical clustering: \citep{Visser2010, gerig2004analysis,
  Guevara2010, zhang2005dti}

Ancillary techniques: \citet{Guevara2010} (partitioning tractography by
track length)

\citet{Tsai2007}: minimum spanning trees, locally linear embedding and
k-means.
...
\citet{brun2004clustering} graph theoretic segmentation

\citet{Ding2003a}:k-nearest neighbours (agglomerative)

\citet{corouge2004towards, Corouge2004, Corouge2006}: Generalized
...
\citet{Durrleman2009,Durrleman2009, durrleman2010registration}:
electrical current models, used for registration of bundles as shown in
\citet{}. Point-to-point or fibre-to-fibre correspondences are not
required, however it is sensitive to fibre density and orientation of
the bundles and it is computationally expensive.

\citet{leemans17new, malcolm2009filtered}: affinity propagation applied
to a small or filtered tractographies, though doubtful about application
to group tractography..

\citet{ziyan2009consistency}: probabilistic registration and clustering
algorithm based on expectation maximization (EM) to create an atlas from
a set of subjects on three bundles iteratively relabeled.
...
\citet{brun2004clustering} feature space based on track mean and
covariance

\citet{wang2010tractography}: each track is modeled as a discrete
